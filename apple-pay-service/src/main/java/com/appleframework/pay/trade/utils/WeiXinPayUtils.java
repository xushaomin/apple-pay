...
public class WeiXinPayUtils {
     ...
     * @return
     */
    public static String geWeiXintPrePaySign(String appid , String mch_id , String device_info , 
    		String trade_type , Map<String, Object> prePay , String partnerKey){
	...
	public static Map<String, String> getPrePayMapForAPP(Map<String, Object> prePayRequest, String partnerKey) {
		...
		prePayMap.put("partnerid", prePayRequest.get("mch_id").toString()); // 商户号
		prePayMap.put("prepayid", prePayRequest.get("prepay_id").toString()); // 预支付交易会话ID
		prePayMap.put("noncestr", prePayRequest.get("nonce_str").toString()); // 随机字符串

		prePayMap.put("package", "Sign=WXPay"); // 扩展字段
		prePayMap.put("timestamp", String.valueOf(System.currentTimeMillis()).substring(0, 10)); // 时间戳

		prePayMap.put("sign", getPrePaySignForApp(prePayMap, partnerKey)); // 扩展字段
		return prePayMap;
	}
