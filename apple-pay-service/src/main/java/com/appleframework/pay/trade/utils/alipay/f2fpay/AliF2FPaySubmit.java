...
//    }
     ...
     * @param amount    支付金额
     * @param goodsDetailses
     */
    public Map<String , String > f2fPay(String outTradeNo , String subject , String body ,String authCode , BigDecimal amount , List<GoodsDetails> goodsDetailses) {
        Map<String , String > returnMap = new HashMap<String , String >();

        String totalAmount = amount.toString()  ;//订单金额
        String undiscountableAmount = "0.0";//默认折扣金额为0,建议由业务系统记录折扣金额,值传递给支付宝实际支付金额
        String storeId = "apple_pay_store_id"; // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        ...
        if (goodsDetailses != null && goodsDetailses.size() > 0){
            for (GoodsDetails goodsDetails : goodsDetailses){
                // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
                GoodsDetail goods = GoodsDetail.newInstance(goodsDetails.getGoodsId(), goodsDetails.getGoodsName(), goodsDetails.getSinglePrice(), goodsDetails.getNums());
                // 创建好一个商品后添加至商品明细列表
